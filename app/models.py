...
class User(UserMixin, db.Model):
    ...
    def generate_fake(count=100):
        ...
        for i in range(count):
            ...
            db.session.add(u)
            ##It's possible that a user might be generated randomly that already
            ##has been entered, therefore integrity error will deal with that
            ## and roll back
            ## This might result in less than the actual count be submitted.
            try:
                db.session.commit()
    ...
    def can(self, permissions):
        ...
        and the permissions assigned to the role.
        """
    ...
    def followed_posts(self):
        """ Obtain followed posts by a join on followed_id and author_id. """
...
class Comment(db.Model):
    ...
    def on_changed_body(target, value, oldvalue, initiator):
        allowed_tags = ['a', 'abbr', 'acronym', 'b', 'blockquote', 'code',
                        'em', 'i']
        target.body_html = bleak.linkify(bleach.clean(
            markdown(value, output_format='html'),
            tags=allowed_tags, strip=True))
